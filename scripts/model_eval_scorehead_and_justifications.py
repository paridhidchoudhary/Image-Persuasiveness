...
def initialize_model():
    ...
    return model, score_head, processor, device, tokenizer


# ── Stage 1: get scores from score head ───────────────────────────────────────
def get_scores_stage1(model, score_head, processor, device, images, inputs):
    """
    Extract per-image scores from the score head using vision token hidden states.
    inputs: already processed and on device.
    Returns: (scores list, hidden_states) — hidden states reused in stage 2.
    """
    model_dtype = next(
        (p.dtype for p in model.parameters() if p.dtype in (torch.float16, torch.bfloat16)),
        torch.float16
...
def get_model_outputs(model, score_head, processor, tokenizer, device,
    ...
    try:
        ...
        # ── Stage 1: scores ────────────────────────────────────────────────────
        scores = get_scores_stage1(model, score_head, processor, device, images, inputs)
...
def analyze_model_performance():
    ...
    model, score_head, processor, device, tokenizer = initialize_model()
