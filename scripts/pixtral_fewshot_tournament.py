...
debug_output_dir = os.path.join(data_root, "tournament_detailed_logs_pixtral_fewshot")
...
# Initialize Mistral API for Pixtral
...
print(f"Pixtral model initialized successfully!")
...
def extract_fewshot_output(file_path):
    ...
    print(f"Split position: {split_position}")
...
def get_pairwise_ranking(image_pair, category, fewshot_example=None, round_name="", debug_info=None):
    """Get Pixtral model prediction for a strictly pairwise comparison of images with detailed debugging."""
    ...
    try:
        # Format image paths for Pixtral (base64 encoding)
        formatted_images = []
        ...
        if fewshot_example:
            ...
            for img_path in fewshot_example['images']:
                try:
                    ...
                    print(f"Warning: Error encoding example image {img_path}: {str(e)}")
                    # Continue even if an example image fails
                    continue
            ...
            # Add separator and instruction for new images
            content.append({
                "type": "text",
                "text": f"\nNow, evaluate these two images in the '{category}' product category. "
        ...
        # Create user message for Mistral API
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Use our extraction function that handles few-shot examples
                ...
                if len(extracted_info) != 2:
                    ...
                    if len(extracted_info) > 2 and fewshot_example:
                        ...
                        # Still not 2 entries? Try next attempt or return None
                        if len(extracted_info) != 2:
                            if debug_info is not None:
                                round_info["status"] = "failed"
                                round_info["error"] = error_msg
                ...
                if attempt < max_retries - 1:
                    ...
                    error_msg = f"All retry attempts failed for {category} ({round_name}): {str(e)}"
                    print(f"{error_msg}")
                    if debug_info is not None:
                        round_info["status"] = "failed"
                        round_info["error"] = error_msg
...
def conduct_tournament(images, category, group, debug_info, fewshot_example=None):
    ...
    if num_images == 2:
        ...
        result = get_pairwise_ranking(images, category, 
    ...
    # For 3 images, use tournament structure
        ...
        first_round = get_pairwise_ranking(images[:2], category, 
        ...
        final_round = get_pairwise_ranking(finalist_images, category, 
        ...
        semi1 = get_pairwise_ranking(images[:2], category, 
        ...
        semi2 = get_pairwise_ranking(images[2:], category, 
...
def evaluate_tournament_with_detailed_logging(data):
    ...
    for batch in tqdm(test_data, desc="Processing evaluation groups"):
        ...
        # Output file paths
        metrics_file = os.path.join(group_dir, "output_pixtral_fewshot.txt")
        ...
        try:
            ...
            final_match, tournament_brackets, finalist_indices = conduct_tournament(
                images, category, group, debug_info, fewshot_example
            )
    ...
    if stats["success"] > 0:
        ...
        # Write summary file
        summary_file = os.path.join(debug_output_dir, "pixtral_fewshot_summary.txt")
        with open(summary_file, "w", encoding="utf-8") as f:
            f.write("PIXTRAL FEW-SHOT TOURNAMENT EVALUATION SUMMARY\n")
            f.write("=" * 60 + "\n\n")
...
if __name__ == "__main__":
    data = load_dataset()
    evaluate_tournament_with_detailed_logging(data)