"""
Phase 2 Training: Joint LM + Ranking Loss

Loads the phase 1 checkpoint (score head only, frozen backbone),
unfreezes LoRA layers, and trains jointly with:
  loss = 0.05 * lm_loss + 0.95 * rank_loss

The score head is already good from phase 1, so LM loss only
fine-tunes backbone representations slightly without dominating.

Prerequisites:
  - Phase 1 training completed
  - Checkpoint saved at PHASE1_MODEL_PATH (score_head.pt + LoRA weights)
"""
...
# ── Paths ──────────────────────────────────────────────────────────────────────
# Phase 1 checkpoint — where score_head.pt and LoRA weights are saved
PHASE1_MODEL_PATH = "./vlm_finetuned_listwise_scorehead_only_split0.15"
# Phase 2 output — saved separately so phase 1 checkpoint is preserved
PHASE2_MODEL_PATH = "./vlm_finetuned_listwise_lm_phase2"
...
# ── LM loss weight — keep very small so ranking loss still dominates ───────────
LM_LOSS_WEIGHT   = 0.05
RANK_LOSS_WEIGHT = 0.95
...
# ── Recreate score head (must match phase 1 exactly) ──────────────────────────
...
# Load score head weights from phase 1
score_head_path = os.path.join(PHASE1_MODEL_PATH, "score_head.pt")
if not os.path.exists(score_head_path):
    raise FileNotFoundError(
        f"Phase 1 score head not found at {score_head_path}. "
        f"Make sure phase 1 training completed and saved correctly."
    )
...
lora_path = os.path.join(PHASE1_MODEL_PATH, "adapter_model.safetensors")
if os.path.exists(lora_path):
    lora_weights = st.load_file(lora_path, device=str(model.device))
    set_peft_model_state_dict(model, lora_weights)
    print(f"✓ Phase 1 LoRA weights loaded from {lora_path}")
else:
    # Try bin format
    lora_bin = os.path.join(PHASE1_MODEL_PATH, "adapter_model.bin")
    if os.path.exists(lora_bin):
        lora_weights = torch.load(lora_bin, map_location=model.device)
        set_peft_model_state_dict(model, lora_weights)
        print(f"✓ Phase 1 LoRA weights loaded from {lora_bin}")
    else:
        print("⚠ No LoRA weights found from phase 1 — starting LoRA from scratch")

# ── Phase 2: unfreeze LoRA + score head, freeze rest ──────────────────────────
# Unfreeze LoRA parameters (these have 'lora_' in their name)
lora_params = 0
...
score_head_params = sum(p.numel() for p in model.score_head.parameters())
print(f"✓ Unfroze {lora_params:,} LoRA params + {score_head_params:,} score head params")
print(f"  Total trainable: {lora_params + score_head_params:,}")
...
# Use SAME split as phase 1 so train/test sets are identical
...
def collate_fn(batch):
    ...
    try:
        ...
        # Encode prompt only (for forward pass inputs)
        inputs = processor(
            text=prompt_text,
            images=image_inputs,
            videos=video_inputs,
        ...
        # Encode full text (for labels)
        labels_ids = processor(
            text=full_text,
            padding=True,
            truncation=True,
            max_length=2048,
        ...
        # Mask prompt tokens so LM loss only trains on assistant response
        labels_ids = torch.where(
...
# ── Loss functions (same as phase 1) ──────────────────────────────────────────
...
class Phase2Trainer(Trainer):
    ...
    def create_optimizer(self):
        # Optimize both LoRA params and score head with same lr
        # Use a slightly lower lr than phase 1 to avoid catastrophic forgetting
        ...
        self.optimizer = torch.optim.AdamW(
            [
                ...
                {"params": score_head_params, "lr": self.args.learning_rate * 2},
            ],
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if labels is not None:
            ...
            shift_labels = labels[..., 1:].contiguous().to(model.device)
        ...
        if len(image_embeddings) != num_images:
            print(f"Warning: Could not extract {num_images} image embeddings, using LM loss only")
            # Still backprop LM loss even if ranking fails
        ...
        # ── Combined loss ─────────────────────────────────────────────────────
        loss = LM_LOSS_WEIGHT * lm_loss + RANK_LOSS_WEIGHT * rank_loss

        if self.state.global_step % 50 == 0:
            print(f"\n=== Step {self.state.global_step} ===")
            print(f"LM Loss:    {lm_loss.item():.4f}  (weight={LM_LOSS_WEIGHT})")
            print(f"Rank Loss:  {rank_loss.item():.4f}  (weight={RANK_LOSS_WEIGHT})")
...
# ── Training args — lower lr and fewer epochs than phase 1 ────────────────────
training_args = TrainingArguments(
    output_dir=PHASE2_MODEL_PATH,
    overwrite_output_dir=True,
    per_device_train_batch_size=1,
    gradient_accumulation_steps=4,
    num_train_epochs=5,           # fewer epochs — score head already converged
    learning_rate=2e-5,           # lower lr — avoid catastrophic forgetting
...
print("\nStarting Phase 2 training (LoRA + score head + LM loss)...")
print(f"LM loss weight: {LM_LOSS_WEIGHT}  |  Rank loss weight: {RANK_LOSS_WEIGHT}")
...
model.save_pretrained(PHASE2_MODEL_PATH)
processor.save_pretrained(PHASE2_MODEL_PATH)

score_head_path = os.path.join(PHASE2_MODEL_PATH, "score_head.pt")
