...
debug_output_dir = os.path.join(data_root, "tournament_detailed_logs")
...
#model_name = "Deb123/qwen2.5-vl-7b-final-finetuned-private"
#model_name = "vlm_finetuned"
...
def get_model_ranking(model, processor, images, category, round_name="", debug_info=None):
    ...
    try:
        # Format image paths using file:// prefix
        formatted_images = []
...
def conduct_tournament(model, processor, images, category, group, debug_info):
    ...
    # For 3 or 4 images, use tournament structure
        ...
        final_round = get_model_ranking(model, processor, finalist_images, category, 
...
def evaluate_tournament_with_detailed_logging(data):
    ...
    for batch in tqdm(test_data, desc="Processing evaluation groups"):
        ...
        # Output file paths
        metrics_file = os.path.join(group_dir, "output_qwen_zeroshot.txt")
        ...
        try:
            ...
            final_match, tournament_brackets, finalist_indices = conduct_tournament(
                model, processor, images, category, group, debug_info
            )
    ...
    if stats["success"] > 0:
        ...
        # Write summary file
        summary_file = os.path.join(debug_output_dir, "qwen_zeroshot_summary.txt")
        with open(summary_file, "w", encoding="utf-8") as f:
            f.write("QWEN ZERO-SHOT TOURNAMENT EVALUATION SUMMARY\n")
            f.write("=" * 60 + "\n\n")
