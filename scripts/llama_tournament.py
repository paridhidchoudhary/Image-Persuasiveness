...
metrics_output_dir = os.path.join(data_root, "dataset_response_for_tournament_llama")
debug_output_dir = os.path.join(data_root, "tournament_detailed_logs_llama")
os.makedirs(metrics_output_dir, exist_ok=True)
os.makedirs(debug_output_dir, exist_ok=True)

# Define Llama model via Groq API
API_KEY = "gsk_xxxxxxxxxxxxxxxx"  # Replace with your Groq API key
MODEL_NAME = "meta-llama/llama-4-scout-17b-16e-instruct"

# Initialize Groq client
client = Groq(api_key=API_KEY)

print(f"Llama model initialized via Groq API!")
...
def get_model_ranking(client, images, category, round_name="", debug_info=None):
    """Get model prediction for a set of images with detailed debugging."""
    ...
    try:
        # Define prompt
        prompt = f"You are evaluating images in the '{category}' product category. Rank the images, based on their appeal for selling this '{category}' product. Provide description, and **persuasion score (1-100)** for each image and explain the ranking."
        ...
        for img_path in images:
            try:
                ...
                error_msg = f"Error encoding image {os.path.basename(img_path)}: {e}"
                print(f"{error_msg}")
                if debug_info is not None:
        ...
        # Call the Groq API for Llama model
...
def evaluate_tournament_with_detailed_logging(data):
    ...
    for batch in tqdm(test_data, desc="Processing evaluation groups"):
        ...
        # Output file paths
        metrics_file = os.path.join(group_dir, "output_llama_tournament.txt")
        ...
        try:
            ...
            with open(debug_summary_file, "w", encoding="utf-8") as f:
                f.write(f"LLAMA TOURNAMENT SUMMARY FOR {category}/{group}\n")
                f.write("=" * 60 + "\n\n")
    ...
    if stats["success"] > 0:
        ...
        # Write summary file
        # Write summary file
        summary_file = os.path.join(debug_output_dir, "llama_zeroshot_summary.txt")
        with open(summary_file, "w", encoding="utf-8") as f:
            f.write("llama ZERO-SHOT TOURNAMENT EVALUATION SUMMARY\n")
            f.write("=" * 60 + "\n\n")
